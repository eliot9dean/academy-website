...
export default function AdminFinancePage() {
  ...
  const classRows = [...classes]
    ...
    .map(cls => ({
      ...
      subject: cls.subject, teacher: users.find(u => u.id === cls.teacherId)?.name ?? '-',
    }));
  ...
  const fixRecs = useMemo(() => periodFin.filter(f => f.type === 'fixed_expense').sort((a, b) => b.date.localeCompare(a.date)), [periodFin]);
  const varRecs = useMemo(() => periodFin.filter(f => f.type === 'variable_expense').sort((a, b) => b.date.localeCompare(a.date)), [periodFin]);
  ...
  const insights = useMemo(() => {
    ...
    if (income === 0) {
      list.push({ type: 'info', text: '선택 기간에 재무 데이터가 없습니다. DB에 해당 기간 내역을 추가해 주세요.' });
      return list;
    }

    // 영업이익률
    if (margin >= criteria.marginGood) list.push({ type: 'good', text: `영업이익률 ${margin}% — 수익 구조가 안정적입니다. 현 비용 구조를 유지하세요.` });
    else if (margin >= criteria.marginWarn) list.push({ type: 'warn', text: `영업이익률 ${margin}% — 개선 여지가 있습니다. 변동비(현재 ${formatKRW(variable)}) 절감을 검토하세요.` });
    else list.push({ type: 'bad', text: `영업이익률 ${margin}% — 매우 낮습니다. 고정비·변동비 전면 재검토가 필요합니다.` });

    // 전기 대비 변화
    if (momIncome !== null) {
      if (momIncome > 0) list.push({ type: 'good', text: `전기 대비 수입 +${momIncome}% 증가 (${formatKRW(prevIncome)} → ${formatKRW(income)}). 성장 추세를 유지하세요.` });
      else if (momIncome < 0) list.push({ type: 'warn', text: `전기 대비 수입 ${momIncome}% 감소 (${formatKRW(prevIncome)} → ${formatKRW(income)}). 신규 등록 유치 강화가 필요합니다.` });
      else list.push({ type: 'info', text: `전기 대비 수입 변동 없음 (${formatKRW(income)}). 성장 전략 수립이 필요합니다.` });
    }
    if (momFixed !== null && momFixed > 5) {
      list.push({ type: 'warn', text: `전기 대비 고정비 +${momFixed}% 증가 (${formatKRW(prevFixed)} → ${formatKRW(fixed)}). 고정비 항목별 원인 파악이 필요합니다.` });
    }
    if (momNet !== null && momNet < -10) {
      list.push({ type: 'bad', text: `전기 대비 순이익 ${momNet}% 급감 (${formatKRW(prevNet)} → ${formatKRW(net)}). 즉각적인 원가 구조 점검이 필요합니다.` });
    }

    // 손익분기점
    if (breakEvenCount > 0) {
      if (breakEvenStatus) list.push({ type: 'good', text: `손익분기점 달성 ✓ — 재원생 ${enrolledCount}명 (기준 ${breakEvenCount}명). 추가 학생은 순이익에 직결됩니다.` });
      else list.push({ type: 'bad', text: `손익분기점 미달 — ${breakEvenCount - enrolledCount}명 추가 모집 시 흑자 전환 가능 (현재 ${enrolledCount}명 / 기준 ${breakEvenCount}명).` });
    }
    ...
    else if (withdrawalRate <= criteria.withdrawWarn) list.push({ type: 'warn', text: `이탈률 ${withdrawalRate}% (${withdrawnInPeriod.length}명 퇴원) — 학부모 소통 강화 및 퇴원 사유 분석을 권장합니다.` });
    else list.push({ type: 'bad', text: `이탈률 ${withdrawalRate}%로 위험 수준 — 즉각적인 원인 분석과 학부모 만족도 점검이 필요합니다.` });

    // 순증감
    if (netChange > 0) list.push({ type: 'good', text: `기간 내 학생 순증감 +${netChange}명 (신규 ${newInPeriod.length}명 / 퇴원 ${withdrawnInPeriod.length}명).` });
    else if (netChange < 0) list.push({ type: 'warn', text: `기간 내 학생 순감소 ${netChange}명 (신규 ${newInPeriod.length}명 / 퇴원 ${withdrawnInPeriod.length}명) — 신규 모집 강화가 필요합니다.` });

    // ARPU
    if (arpu > 0) list.push({ type: 'info', text: `학생 1인당 평균 수강료(ARPU): ${formatKRW(arpu)} — 수강료 단가 조정 시 수익 변동 참고 지표입니다.` });

    // 미납 (추정 미수금 포함)
    if (unpaidList.length > 0) {
      const est = arpu > 0 ? unpaidList.length * arpu : 0;
      list.push({ type: 'warn', text: `수강료 미납 ${unpaidList.length}명 (${unpaidList.map(s => s.name).join(', ')})${est > 0 ? ` — 추정 미수금 ${formatKRW(est)}` : ''}. 미수금 회수 조치가 필요합니다.` });
    }

    // 마케팅 효율
    if (sourceData.length > 0) {
      const top = sourceData[0];
      list.push({ type: 'info', text: `최고 효율 유입 채널: "${top.source}" — 전환율 ${top.rate}% (상담 ${top.total}건 → 등록 ${top.reg}명). 해당 채널 집중 투자를 검토하세요.` });
    }

    // 고정비 비율
    const fixedRatio = income > 0 ? Math.round(fixed / income * 100) : 0;
    if (fixedRatio > criteria.fixedBad) list.push({ type: 'bad', text: `고정비 비율 ${fixedRatio}% — 수입 대비 고정비가 너무 높습니다. 인건비·임차료 구조 재검토를 권장합니다.` });
    else if (fixedRatio > criteria.fixedWarn) list.push({ type: 'warn', text: `고정비 비율 ${fixedRatio}% — 다소 높은 편입니다. 재원생 수 확대로 고정비 레버리지 효과를 노리세요.` });

    // 선생님 부담 편차
    if (teacherRows.length > 1) {
      const loads = teacherRows.map(t => t.students);
      const maxLoad = Math.max(...loads), minLoad = Math.min(...loads);
      if (maxLoad - minLoad > 5) {
        list.push({ type: 'info', text: `선생님별 학생 배분 편차가 큽니다 (최대 ${maxLoad}명 vs 최소 ${minLoad}명). 수업 재편성을 고려하세요.` });
      }
    }
    ...
    return list;
      ...
      momIncome, momFixed, momNet, prevIncome, prevFixed, prevNet, net, teacherRows]);
              ...
              {editingId === item.id ? (
                  ...
                  <div className="grid grid-cols-2 gap-2 mb-2.5">
                    <div>
                      ...
                      <input type="date" className="w-full px-2 py-1.5 rounded-lg text-xs"
                        ...
                        onChange={e => setDraftEdit(p => ({ ...p, date: e.target.value }))} />
                    </div>
                    <div>
                      ...
                      <select className="w-full px-2 py-1.5 rounded-lg text-xs"
                        ...
                        onChange={e => setDraftEdit(p => ({ ...p, category: e.target.value }))}>
                    ...
                    <div>
                      ...
                      <input type="number" className="w-full px-2 py-1.5 rounded-lg text-xs"
                        style={{ border: '1px solid #C7D2FE', outline: 'none', color: '#334155' }}
                        value={draftEdit.amount ?? ''}
                        onChange={e => setDraftEdit(p => ({ ...p, amount: Number(e.target.value) }))} />
                    </div>
                    <div>
                      ...
                      <input className="w-full px-2 py-1.5 rounded-lg text-xs"
                        ...
                        onChange={e => setDraftEdit(p => ({ ...p, description: e.target.value }))} />
                    </div>
                  </div>
                  <div className="flex gap-1.5 justify-end">
                    <button onClick={() => deleteRec(item.id)}
                      className="text-[11px] px-2.5 py-1 rounded-lg"
                      style={{ background: '#FEF2F2', color: '#EF4444' }}>삭제</button>
                ...
                <div onClick={() => startEdit(item)}
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    {item.description && (
                      <span className="ml-2 text-xs" style={{ color: '#94A3B8' }}>{item.description}</span>
                    )}
  ...
  return (
    <div>
      ...
      <div className="page-header flex items-center justify-between flex-wrap gap-3">
        <div>
          <h1 className="page-title">재무현황</h1>
          <p className="page-subtitle">{periodLabel} · 수익성 분석 및 운영 인사이트</p>
        </div>

        {/* 기간 선택 (우상단) */}
        <div className="flex items-center gap-2 flex-wrap">
          {/* 연도 선택 */}
          <select className="form-select text-xs font-semibold"
            value={year} onChange={e => { setYear(Number(e.target.value)); setMonthSel(`${e.target.value}-04`); }}>
                ...
                <div className="absolute left-1/2 -translate-x-1/2 top-6 z-50 w-[340px] rounded-xl shadow-xl p-4 text-xs"
                  ...
                  <table className="w-full border-collapse" style={{ fontSize: 10.5 }}>
                    ...
                    <tbody style={{ color: '#CBD5E1' }}>
                      <tr className="border-t border-white/10"><td className="py-1 pr-2">영업이익률</td><td className="text-center">≥ {criteria.marginGood}%</td><td className="text-center">{criteria.marginWarn}~{criteria.marginGood}%</td><td className="text-center">&lt; {criteria.marginWarn}%</td></tr>
                      <tr className="border-t border-white/10"><td className="py-1 pr-2">고정비 비율</td><td className="text-center">≤ {criteria.fixedWarn}%</td><td className="text-center">{criteria.fixedWarn}~{criteria.fixedBad}%</td><td className="text-center">&gt; {criteria.fixedBad}%</td></tr>
                      <tr className="border-t border-white/10"><td className="py-1 pr-2">이탈률</td><td className="text-center">≤ {criteria.withdrawInfo}%</td><td className="text-center">{criteria.withdrawInfo}~{criteria.withdrawWarn}%</td><td className="text-center">&gt; {criteria.withdrawWarn}%</td></tr>
                      <tr className="border-t border-white/10"><td className="py-1 pr-2">손익분기점</td><td className="text-center">달성</td><td className="text-center" colSpan={2}>미달성</td></tr>
                    </tbody>
                  </table>
                  {/* 기준 편집 버튼 */}
                  <button
        ...
        <div className="space-y-2 mb-3">
          {insights.map((ins, i) => (
            <div key={i} className="flex items-start gap-2 rounded-lg px-3 py-2 text-xs"
              style={{ background: INSIGHT_BG[ins.type] }}>
        ...
        <div style={{ borderTop: '1px solid #E2E8F0', paddingTop: 12 }}>
          ...
          <div className="relative">
            <textarea className="w-full px-3 py-2 rounded-lg text-sm resize-none"
              style={{ border: '1px solid #E2E8F0', outline: 'none', minHeight: 64, color: '#334155' }}
              placeholder="클릭하여 메모 입력... (저장: Ctrl+Enter)"
        ...
        <div className="card p-4">
          ...
          <div className="space-y-3">
            <div className="flex justify-between text-sm">
              <span style={{ color: '#64748B' }}>고정비</span>
              <span className="font-semibold" style={{ color: '#F97316' }}>{formatKRW(fixed)}</span>
            </div>
            <div className="flex justify-between text-sm">
              <span style={{ color: '#64748B' }}>1인당 수익 (ARPU)</span>
              <span className="font-semibold" style={{ color: '#2563EB' }}>{arpu > 0 ? formatKRW(arpu) : '—'}</span>
            </div>
              ...
              <div>
                <div className="h-2.5 rounded-full overflow-hidden mt-1" style={{ background: '#F1F5F9' }}>
                  <div className="h-full rounded-full transition-all"
                    style={{ width: `${Math.min(100, Math.round(enrolledCount / breakEvenCount * 100))}%`,
                      background: breakEvenStatus ? '#22C55E' : '#F59E0B' }} />
                </div>
                <div className="text-xs mt-1 text-right" style={{ color: '#94A3B8' }}>
                  {Math.round(enrolledCount / breakEvenCount * 100)}% 달성
                  {!breakEvenStatus && ` · ${breakEvenCount - enrolledCount}명 부족`}
                </div>
              </div>
            )}
            {/* 비용 구조 비율 */}
            {income > 0 && (
              <div className="pt-2 border-t" style={{ borderColor: '#F1F5F9' }}>
                ...
                <div className="flex justify-between text-[9px] mt-1" style={{ color: '#94A3B8' }}>
                  <span>고정비 {income > 0 ? Math.round(fixed/income*100) : 0}%</span>
                  <span>변동비 {income > 0 ? Math.round(variable/income*100) : 0}%</span>
            ...
            {teacherRows.length === 0 ? (
              ...
              return (
                <div key={t.name}>
                  ...
                  {stuIncome > 0 && (
                    <div className="text-[10px] mt-0.5 text-right" style={{ color: '#94A3B8' }}>
                      담당 수강료 기여 {formatKRW(stuIncome)}
                    </div>
                  )}
                </div>
              );
          ...
          <div className="space-y-2 max-h-[200px] overflow-y-auto">
            {classRows.map(cls => (
              <div key={cls.name} className="flex items-center gap-2 text-xs">
                ...
                <span className="font-bold" style={{ color: cls.students >= 8 ? '#16A34A' : cls.students >= 5 ? '#F59E0B' : '#EF4444' }}>
                  {cls.students}명
                </span>
              </div>
            ))}
          ...
          <div className="overflow-x-auto">
            <table className="data-table w-full">
              <thead>
                <tr>
                  <th>유입 채널</th>
                  <th className="text-center">상담 수</th>
                  <th className="text-center">등록 수</th>
                  ...
                  <tr key={s.source}>
                    ...
                    <td className="text-center">
                      <span className="text-xs font-bold px-2 py-0.5 rounded-full"
                        style={{ background: s.rate >= 50 ? '#DCFCE7' : s.rate >= 30 ? '#FEF9C3' : '#FEE2E2',
                          color: s.rate >= 50 ? '#15803D' : s.rate >= 30 ? '#92400E' : '#991B1B' }}>
                        {s.rate}%
                      </span>
                    </td>
                    <td className="text-xs" style={{ color: '#94A3B8' }}>
                      {s.rate >= 50 ? '🟢 집중 투자 권장' : s.rate >= 30 ? '🟡 유지' : '🔴 효율 개선 필요'}
                    </td>
                  </tr>
                ))}
              </tbody>
      ...
      {showCriteriaEdit && (
        <div className="fixed inset-0 bg-black/40 z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl p-6 w-full max-w-md shadow-xl">
            ...
            <p className="text-xs mb-4" style={{ color: '#94A3B8' }}>저장 시 AI 인사이트 평가에 즉시 반영됩니다. 스탭 페이지와 동일한 기준이 공유됩니다.</p>
            <div className="space-y-4">
              <div>
                <div className="text-xs font-semibold mb-1" style={{ color: '#475569' }}>영업이익률</div>
              ...
              <div>
                <div className="text-xs font-semibold mb-1" style={{ color: '#475569' }}>고정비 비율</div>
              ...
              <div>
                <div className="text-xs font-semibold mb-1" style={{ color: '#475569' }}>이탈률</div>
